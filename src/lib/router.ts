...
export const router = createRouterFromEnv({
  ...
  guidance: [
    ...
    "Files:    GET /api/statuslines/{slug}/files — companion Claude Code command files (agent-executed PROMPTS; read each with the user before saving under ~/.claude/commands/).",
    ...
    "Buy:      POST /api/download with {\"slug\": \"...\"} — pays the creator's asking price straight to their wallet; returns the script + any companion files + install steps. Review before installing.",
    "Publish:  POST /api/register ($0.15, funds the audit) with `script` + `previewAnsi` + slug/name/description/priceUsd (0 = free, or any amount)/tags, optionally `files` (≤3 commands/<name>.md command files — audited with the script). A failed audit is not listed (fee bought the audit). Proceeds pay the registering wallet. Connect an X identity to show @handle as author: POST /api/identity/connect (SIWX, free) returns an authorizeUrl — the user opens it in a browser and signs in with X; done.",
    "Update:   POST /api/statuslines/{slug}/update ($0.15, owner wallet only) with `script` + optional previewAnsi/previewFrames/files — re-runs the full audit on the replacement; a reject leaves the existing listing live and unchanged.",
